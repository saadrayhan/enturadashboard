...
export default function ContactDetailPage() {
  ...
  const details = [
    ...
    { icon: Phone, label: "Phone", value: contact.phone, action: () => copyToClipboard(contact.phone, "Phone"), mono: true },
    ...
    { icon: Calendar, label: "Created", value: contact.dateCreated, mono: true },
  ];
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 p-4 sm:p-6 space-y-6 overflow-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted text-sm font-semibold text-muted-foreground shrink-0">
              {contact.avatar}
            </div>
            <div>
          ...
          <div>
            <h2 className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground mb-4">Activity</h2>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex gap-3">
                  <div className={`mt-1.5 h-2.5 w-2.5 rounded-full ${item.dot} shrink-0`} />
                  <div>
      ...
      <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-border bg-card p-4 sm:p-6 space-y-5">
        <h2 className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Contact details</h2>
        <div className="space-y-4">
          {details.map((d) => (
            <div key={d.label} className="flex items-start gap-3 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                {d.isLink ? (
                  <a href={`https://linkedin.com/in/${d.value}`} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline">
                    {d.value}
                  </a>
                ) : (
                  <p className={`text-sm text-foreground truncate ${d.mono ? "font-mono" : ""}`}>{d.value}</p>
                )}
              </div>
              {d.action && (
