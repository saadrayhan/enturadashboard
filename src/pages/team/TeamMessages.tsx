...
export default function TeamMessages() {
  ...
  useEffect(() => {
    ...
    load();
    const channel = supabase.channel("team-msgs").on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `project_id=eq.${selectedProject}` }, () => load()).subscribe();
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 h-[calc(100vh-3rem)] flex flex-col">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Communicate with team and clients</p>
      </div>
      ...
      <Card className="flex-1 flex flex-col min-h-0">
        <CardContent className="flex-1 overflow-auto p-4 space-y-3">
          {!selectedProject ? (
            <p className="text-sm text-muted-foreground text-center py-8">Select a project</p>
          ) : messages.length === 0 ? (
